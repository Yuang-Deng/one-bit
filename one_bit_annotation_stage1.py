...
from main_stage0 import CIFAR100DS, CIFAR10DS
...
def one_bit_super_stage1(pred_list, target_list, idx_list, num_query=27000):
    ind_sort_chosed = range(num_query)
...
def one_bit_super_stage2(pred_list, target_list, idx_list, num_query=20000):
    ind_sort_chosed = np.random.permutation(len(pred_list))[0:num_query]
    ind_predTrue = np.where(pred_list[ind_sort_chosed] == target_list[ind_sort_chosed])[0]
    ind_predFalse = np.where(pred_list[ind_sort_chosed] != target_list[ind_sort_chosed])[0]
...
def read_cifar100():
    ...
    traindir = 'C:/Users/Alex/WorkSpace/pycharm/mymoco/datasets'
    dataset = CIFAR100DS(traindir, transform=eval_transformation)
    return dataset
...
def read_cifar10():
    ...
    traindir = 'C:/Users/Alex/WorkSpace/pycharm/mymoco/datasets'
    dataset = CIFAR10DS(traindir, transform=eval_transformation)
    return dataset
...
def main():
    ...
    unlabeled_idxs = read_dataset_initial(dataset, 30)
    ...
    #labeled_feats = pred_labeled_data(labeled_idxs_initial, dataset, model)

    pred_true, pred_false, false_pred = one_bit_super_stage1(top1_pred_list, traget_list, idx_list, num_query=num_query)
    #print(len(pred_true), len(pred_false), len(false_pred))

    #pdb.set_trace()
    os.makedirs('./index_' + ds + '/rand_select', exist_ok=True)
    #os.makedirs('index_cifar100/3000initial/three_stage', exist_ok=True)
    np.save('./index_' + ds + '/rand_select/pred_true_stage1.npy', pred_true)
    np.save('./index_' + ds + '/rand_select/pred_false_stage1.npy', pred_false)
    np.save('./index_' + ds + '/rand_select/false_pred_stage1.npy', false_pred)
