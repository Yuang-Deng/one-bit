...
class CIFAR10DS(torch_datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, download=False):
        super().__init__(root, train=train, transform=transform, download=download)
        self.imgs = []
        for data, label in zip(self.data, self.targets):
            self.imgs.append([data, label])
...
class CIFAR100DS(torch_datasets.CIFAR100):
    def __init__(self, root, train=True, transform=None, download=False):
        super().__init__(root, train=train, transform=transform, download=download)
        self.imgs = []
        for data, label in zip(self.data, self.targets):
...
def save_checkpoint(state, is_best, dirpath, epoch):
    ...
    checkpoint_path = os.path.join(dirpath, 'stage0', filename)
    best_path = os.path.join(dirpath,'stage0', 'best_initial.ckpt')
