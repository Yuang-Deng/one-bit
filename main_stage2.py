...
##### conduct one-bit annotation. #####
import one_bit_annotation_stage2
...
def main(context):
    ...
    def create_model(ema=False, channels=96):
...
def create_data_loaders(train_transformation,
    ...
    dir_path = './index_' + ds + '/rand_select/'
    ...
    pred_false_stage1 = np.load(dir_path+'pred_false_stage1.npy')
    false_pred_stage1 = np.load(dir_path+'false_pred_stage1.npy')

    pred_false_stage2 = np.load(dir_path+'pred_false_stage2.npy')
    false_pred_stage2 = np.load(dir_path+'false_pred_stage2.npy')

    labeled_idxs = np.concatenate((labeled_idxs_initial, labeled_idxs_stage1, labeled_idxs_stage2))
    ...
    labeled_idxs, unlabeled_idxs = data.relabel_dataset_stage2(dataset, labeled_idxs, false_pred_dict1, false_pred_dict2)
...
def train(train_loader, model, ema_model, optimizer, epoch):
    ...
    #for i, ((input, ema_input), target) in enumerate(train_loader):
        ...
        if args.consistency:
            ...
            consistency_loss = consistency_weight * consistency_criterion(cons_logit, ema_logit_new) / minibatch_size
        else:
            consistency_loss = 0
...
def save_checkpoint(state, is_best, dirpath, epoch):
    ...
    checkpoint_path = os.path.join(dirpath, 'stage2', filename)
    best_path = os.path.join(dirpath,'stage2', 'best_stage2.ckpt')
